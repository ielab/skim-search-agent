...
`evaluation/doc_scoring.py` grades a doc answer with EXACT-MATCH + token-F1, gated on grounding.
...
answer-equivalence check, not generation.
    ...
    python -m evaluation.llm_judge --results-dir runs/... --judge-model gpt-4o-mini --dataset musique_structured
    # judge on a served vLLM (cluster, no API key):  vllm serve <judge_model> --port 8001  then:
    python -m evaluation.llm_judge --results-dir runs/... --judge-model <judge_model> \
...
# The BrowseComp Appendix F judge prompt — VERBATIM (identical to BrowseComp-Plus / RISE's own
# scripts/judge.py). Do not paraphrase: fidelity to this exact prompt is what makes the resulting
# accuracy comparable to the published benchmark number instead of a home-grown proxy.
...
def judge_answer_detail(question: str, gold_answer: str, response: str,
    ...
    data = _parse_judge_json(generate(prompt))
...
def judge_run_dir(results_dir: str, generate: Callable[[str], str], *,
                  judge_model: str = "gpt-4o-mini", dataset: Optional[str] = None) -> dict:
    ...
    summary = {"judge_model": judge_model, "judge_prompt": "BrowseComp Appendix F (verbatim)",
               "n_judged": len(graded), "n_correct": n_correct,
               "judge_accuracy": (n_correct / len(graded)) if graded else 0.0}
    (rd / "judge_summary.json").write_text(json.dumps(summary, indent=2))
    return summary
