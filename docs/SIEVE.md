...
| Manual ablation | conditions `agent_research_bql_dense_snip_nomanual` (no manual), `_syntax` (reference only), `_noconstruct` (no query-construction advice); paper prompt, everything else matched | not in the paper; a control grid on the paper's harness found the manual costing Sieve 4.6 to 7.0 points |
