...
Every step is a command. Run them in order from the repository root with the project environment
active. Steps 1 to 4 set up the machine, the data, the indexes and the model server. Step 5 runs one
cell, step 6 runs a whole collection on SLURM, and steps 7 to 9 finish, score and read it. When
step 9 prints your scores, compare them with the tables in [Results](#results). The ITER paper has
its own procedure in [ITER.md](ITER.md).
...
**5.8 The three Sieve rankers as flags.**
...
table above. To regenerate any of it, follow [Reproduce it](#reproduce-it).
