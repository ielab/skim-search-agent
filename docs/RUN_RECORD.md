...
| `prompt_task`, `prompt_strategy`, `prompt_toolset`, `prompt_profile`, `prompt_field_profile`, `prompt_sha256` | the condition (its task, its strategy, the strategy's tool-set name, the task's template file, the dataset's field profile that picked the manuals), plus a hash of the *composed* system prompt (task × tool declarations × manuals) |
