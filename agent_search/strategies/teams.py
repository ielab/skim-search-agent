"""Multi-agent strategies: a procedure whose members are conditions run as agents.

`plan_and_search` is the first team: a planner splits the question, one Sieve agent per
sub-question, a synthesizer answers (`agent_search.procedures.plan_and_search`). The member is
...
plan_and_search = register_strategy(Strategy(
    ...
    loop=False, procedure=PlanAndSearch(searcher="sieve_bm25")))
...
plan_and_search_visit = register_strategy(Strategy(
    ...
    loop=False, procedure=PlanAndSearch(searcher="search_visit")))
