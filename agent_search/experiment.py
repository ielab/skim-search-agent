...
SCHEMA: dict[str, dict[str, Key]] = {
    ...
    "model": {
        ...
        "max_tokens": Key(4000, "env", "LLM_MAX_TOKENS", "generation budget per turn"),
        "max_tokens_schedule": Key(None, "env", "LLM_MAX_TOKENS_SCHEDULE", "per-turn generation budgets, comma separated, the last one repeating (DIVER's Qwen3.5 runs: 4096,2048,1024); null = max_tokens every turn"),
        "thinking": Key(None, "env", "LLM_THINKING", "chat-template thinking switch for a served model (true | false; null = the template's default). A turn cut off mid-thought is discarded and the next turn runs with thinking off, as in DIVER's Qwen3.5 client"),
    ...
    "listing": {
        ...
        "bm25_fetch_topk": Key(5, "env", "BM25_FETCH_TOPK", "results per search, bm25 search->fetch arms"),
        "dense_visit_topk": Key(5, "env", "DENSE_VISIT_TOPK", "results per search, dense search->visit arm"),
        "dense_fetch_topk": Key(5, "env", "DENSE_FETCH_TOPK", "results per search, dense search->fetch arm"),
        "hybrid_visit_topk": Key(5, "env", "HYBRID_VISIT_TOPK", "results per search, hybrid search->visit arm"),
        "hybrid_fetch_topk": Key(5, "env", "HYBRID_FETCH_TOPK", "results per search, hybrid search->fetch arm"),
        ...
        "rerank_fetch_topk": Key(5, "env", "RERANK_FETCH_TOPK", "results per search, reranked search->fetch arm"),
