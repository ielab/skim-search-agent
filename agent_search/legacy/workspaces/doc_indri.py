...
class IndriFetchWorkspace(DocSearchFetch):
    ...
    def __init__(self, units: Sequence[CodeUnit],
        ...
        if executor is not None:
            ...
            # agent_search.retrievers.backend.build_indri_engine. Production
...
register_workspace("indri", tools=("isearch", "fetch"), engines=("indri",),
                   builder=lambda ctx: IndriFetchWorkspace(ctx.units, executor=ctx.indri(), ubyid=ctx.ubyid))
register_workspace("indrivisit", tools=("isearch_v", "visit_v"), engines=("indri",),
                   builder=lambda ctx: IndriVisitWorkspace(ctx.units, executor=ctx.indri(), ubyid=ctx.ubyid))
register_workspace("indrisnip", tools=("isearch_s", "fetch"), engines=("indri",),
                   builder=lambda ctx: IndriFetchWorkspace(ctx.units, executor=ctx.indri(), ubyid=ctx.ubyid, snippets=True))
