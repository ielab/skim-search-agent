...
# --- arms. `code` (search/fetch) is the fallback for a code-domain toolset no other arm claims.
from agent_search.legacy.retriever import register_workspace  # noqa: E402

register_workspace("code", tools=("search", "fetch"), engines=("bql_plain",), domain="code", needs_files=True,
                   builder=lambda ctx: CodeFixWorkspace(ctx.units, ctx.files, executor=ctx.bql_plain(), ubyid=ctx.ubyid))
