# Structured document search v2: field-tagged `term[field]` Boolean + typed date ranges + fetch
...
`date[RANGE]` lowers to a REAL date-range filter over each document's `date` metadata (not a
token match) — reach for it whenever the question gives a year, decade, or an "as of" bound
...
```
search: ... -> AND(foo[title], bar[infobox], date[1980..1989])   (0 exact matches — closest by
CONSTRAINT COVERAGE; 'miss' names the unmatched constraint):
  1  docA  'Some Title'  §[...]  ib[...]  matched: ...  cov=2/3 miss=[date[1980..1989]]
  2  docB  ...                                          cov=1/3 miss=[bar[infobox], date[1980..1989]]
```
...
`[infobox]` clause means that field-scope was too tight — loosen to `[body]`/`[tiab]` or drop
