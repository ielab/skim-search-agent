...
1. Query entity NAMES, never the question's wording — relation words (spouse, owner,
   founder) are what you look FOR in the fetched slice, not what you search for. When the
   question only DESCRIBES a thing (no name given), start from the 1–2 tokens most likely to
   appear VERBATIM in the target doc — a proper name, a domain term, an exact number like
   `1897` — not the framing words (described, mentioned).
2. Pick the hit by the TYPE the question implies, from the listing, not a fetch. Infobox keys
...
(either), `A AND B` (both), `A NOT B`. Parentheses group as expected.
...
entity — search THAT. 0 hits means your SURFACE is wrong, not that the doc is absent —
recover in ONE move: drop a long name to its 1–2 most distinctive words. Two attempts at the
