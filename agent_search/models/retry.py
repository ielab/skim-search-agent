...
`agent_search/agent/forced_answer.py`) goes through `_with_retries` —
exponential backoff + jitter on transient failures (HTTP 429, 5xx, connection
errors, timeouts), re-raising anything else immediately. Attempts/base delay
