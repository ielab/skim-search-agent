...
`agent_search.retrievers.structural.bql.executor.execute_bql`) already call, backed by
`LuceneStructuredEngine` instead of the pure-Python `IndriExecutor`/`StructuralExecutor`.
Constructed ONLY via `agent_search.retrievers.structural.backend`'s `STRUCTURED_BACKEND`
...
from agent_search.retrievers.structural.indri.model import IndriResult
...
class LuceneIndriAdapter:
    ...
    def search(self, query: str, k: int = 5) -> IndriResult:
        query = query or ""
        from agent_search.retrievers.structural.indri.model import _dense_expand_k
    ...
    def _combine_dense(self, hits: list, query: str) -> tuple:
        ...
        from agent_search.retrievers.structural.indri.model import DENSE_NORM_EPS, _dense_weight
...
class LuceneBqlAdapter:
    ...
    def run_with_count(self, expr, k: int = 100) -> tuple:
        ...
        if self.dense is not None and ranked:
            from agent_search.retrievers.structural.bql.executor import _rank_leaves
            from agent_search.retrievers.structural.bql.dense_fuse import fuse_ranked
...
class LuceneBqlDonlyAdapter(LuceneBqlAdapter):
    ...
    subclass instead. See `agent_search/retrievers/structural/bql/dense_fuse.py`'s "dense-ONLY
    ...
    def run_with_count(self, expr, k: int = 100) -> tuple:
        ...
        if self.dense is not None and ranked:
            from agent_search.retrievers.structural.bql.executor import _rank_leaves
            from agent_search.retrievers.structural.bql.dense_fuse import fuse_ranked_dense_only
            leaves = _rank_leaves(expr)
