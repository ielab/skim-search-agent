"""Pure-Python BM25 over code units (no Java / no deps).

Two roles:
  * the `bm25_local` BASELINE (lexical floor; for faithful SWE-bench numbers use
    the Pyserini backend, retrievers/lexical/pyserini.py);
  * the SHARED set->ranking layer: Boolean retrieval returns a SET, so the
    structural executor (and the grep baseline, GrepRAG-style) order their
    candidate sets with THIS scorer — the ranking layer is identical across the
    index-free conditions and only candidate selection differs.

Tokenizer is the shared `agent_search.corpus.units.code_tokenize`.
"""
