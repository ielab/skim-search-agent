...
set unconditionally at this module's import time (below) so `BM25Pyserini` is self-contained
regardless of import order (previously only `evaluation/__init__.py` set it, so importing this
module directly — e.g. from a test or `oneshot_rag.py` — before `evaluation` was imported would
crash on a machine with no `OPENAI_API_KEY` in its environment, even though nothing here ever
calls OpenAI).
"""
...
# See module docstring's "Offline safety" paragraph: a placeholder is enough — pyserini's
# transitive `_openai.py` only needs the env var present at IMPORT time to construct its (never
# used) OpenAI client; no network call happens here or later. `setdefault` so a real key, if one
# happens to be set for an unrelated reason, is left untouched.
...
class BM25Pyserini(Retriever):
    ...
    def index(self, units: Sequence[CodeUnit], key: Optional[str] = None) -> "BM25Pyserini":
        ...
        if self.rebuild or not self._is_built(index_dir, expected_n_docs=len(units)):
            ...
            with open(os.path.join(base, "meta.json"), "w") as fh:
                json.dump({"n_docs": len(units)}, fh)

        with _silence_fd(2):  # swallow the one-time JVM incubator-modules warning
    ...
    @staticmethod
    def _is_built(index_dir: str, expected_n_docs: Optional[int] = None) -> bool:
