"""Structural retrieval backends: `indri/` (Python Dirichlet-smoothed belief
scorer) and `bql/` (Python index-free Boolean executor) are the REFERENCE
engines; `lucene/` (`LuceneStructuredEngine`) is a real-Lucene alternative that
...
workspace actually calls (see `agent_search.retrievers.lucene.adapters`'s module
