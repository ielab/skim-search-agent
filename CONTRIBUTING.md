...
| a model provider | `agent_search/agent/backbone/` (one file per provider; `make_generate` in `__init__.py`) | a matcher and a branch in `make_generate` | [EXTENDING.md §6](docs/EXTENDING.md#6-a-model-provider) |
...
- Length limits are token counts (`agent_search.tokens`). Do not add character-based caps.
