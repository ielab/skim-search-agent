...
conditions run through evaluation/run_eval.py:
  ...
  dense  DenseBelief (agent_search.retrievers.structural.indri.dense_belief), the SAME
         ...
         eval time either).
  hybrid NEW, additive-only: Reciprocal Rank Fusion (RRF, k=`doc_research.RRF_K`=60) of the SAME
         ...
         `agent_search.agent.tools.doc_research.HybridVisit`/`rrf_fuse` use for the
...
# DENSE_MODEL: imported from dense_belief.DEFAULT_MODEL (SAME model DenseVisit/DenseBelief use,
# env `DENSE_MODEL`-overridable — see that module) rather than a second hardcoded literal here.

# BUGFIX (env-configurable completion budget): a hardcoded max_tokens=512 let the Tongyi
# reasoning model burn its whole completion budget inside <think>...</think> and never reach
# <answer> — the row scores empty. Default raised to 4000; override with ONESHOT_MAX_TOKENS.
DEFAULT_MAX_TOKENS = int(os.environ.get("ONESHOT_MAX_TOKENS", "4000"))
# BUGFIX (token accounting): the doc-stuffing budget below MUST be counted in the SAME unit
...
# BUGFIX (residual overflow on 11/200 bm25 rows): SAFETY_MARGIN_TOKENS above only reserves room
...
# — the actual gap — the chat template vLLM applies server-side (role headers, BOS/generation-prompt
# special tokens), none of which are counted against any single doc's cap. On the longest instances
...
def _build_bm25_engine(units: Sequence[CodeUnit], index_root: str = "indexes",
    ...
    use (agent_search/agent/retriever.py) — so a oneshot bm25 run and an agent_research_bm25
...
def _build_hybrid_engine(units: Sequence[CodeUnit], index_root: str = "indexes",
    ...
    """NEW, additive-only: `(bm25_engine, dense_engine)` — the two engines `retrieve_top_k`'s
...
def retrieve_top_k(retriever: str, engine, query: str, k: int = TOP_K) -> list[str]:
    ...
    memoized-encode cosine top-k (same call DenseVisit.search makes); `hybrid` (NEW,
    additive-only): RRF-fuse a bm25 pool and a dense pool, `engine` a `(bm25_engine,
    dense_engine)` pair (see `_build_hybrid_engine`) — SAME `HYBRID_POOL`/`RRF_K`/`rrf_fuse`
    formula `agent_search.agent.tools.doc_research.HybridVisit` uses for research_hybrid,
    ...
    if retriever == "hybrid":
        from agent_search.agent.tools.doc_research import HYBRID_POOL, RRF_K, rrf_fuse
...
# BUGFIX: the stuffing budget used to be counted in whitespace words (`_cap_tokens`-style
# text.split()) while the model's actual limit is BPE tokens — a 120,000-word default budget
# deterministically produced a 130,561-BPE-token prompt on bm25 rows, > 131,072-max_tokens, and
# killed 154/200 rows with a context-overflow error. Fixed by counting/capping in REAL tokens.
...
def stuff_docs(doc_ids: Sequence[str], ubyid: dict,
    ...
    for doc_id in doc_ids:
        ...
        # "full content" bounded by physics: with no per-doc cap, fit the model's context —
        # split a total stuffing budget (env ONESHOT_TOTAL_BUDGET, default derived from
...
def make_generate(model: str, api_base: str, api_key: Optional[str] = None,
    ...
    NOT `agent_search.models.backends.openai_compat_generate` (that helper's
    ...
    to the harness's own deterministic settings (agent_search.models.backends). `max_tokens`
    defaults to DEFAULT_MAX_TOKENS (env ONESHOT_MAX_TOKENS, default 4000) — BUGFIX: a hardcoded
    512 let the Tongyi reasoning model burn its whole completion budget inside <think>...</think>
