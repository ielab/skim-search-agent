...
if [ -z "${JAVA_HOME:-}" ]; then
  echo ">> JAVA_HOME is not set: anything that imports Pyserini (the test suite, every document run) needs JDK 21; pass --export=ALL,JAVA_HOME_OVERRIDE=/path/to/jdk-21" >&2
fi
mkdir -p slurm_logs
