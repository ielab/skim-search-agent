"""Regression tests for episode-termination and tool-call parsing edge cases: answer detection
inside `<think>` blocks, bare-JSON tool-call quirks, search-hit header parsing, qrels TREC-format
tolerance, and the default text-embedder domain."""
import os
import tempfile
...
from agent_search.agent.episode import trajectory_meta as _trajectory_meta
...
def test_qrels_tolerates_space_delimited_trec():
    from agent_search.evaluation.datasets import _qrels_from_tsv
    d = tempfile.mkdtemp()
...
def test_research_domain_uses_text_embedder_default():
    ...
    from agent_search.evaluation.config import DatasetArgs, RetrieverArgs, RunConfig
