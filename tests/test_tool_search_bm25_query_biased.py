...
that fairness fix is what `snippet=TermWindow()` provides, tested directly here since the
condition itself is gone.

`snippet=TermWindow()` keeps the plain listing's bm25 retrieval and whole-doc `visit` read
...
`snippet=OpeningLine()` (the default) reproduces the plain rendering byte-for-byte; the existing
...
from agent_search.snippets import OpeningLine, TermWindow
from agent_search.snippets import TermWindow as _TW
best_line = lambda u, terms, width=32: _TW().render(u, terms, width=width)  # noqa: E731
...
class _StubBm25:
    """A stand-in for the BM25 engine (`BM25Pyserini`) exposing only `search(query, k)` -> ranked doc_ids."""
...
def _toolbox(ranking=("d_mid",), snippet=OpeningLine()):
    units = _units()
    ubyid = {u.doc_id: u for u in units}
    state = EpisodeState(question="q")
    name_search, name_visit = ("bm25q_search", "visit_q") if snippet.shows_excerpt and isinstance(snippet, _TW) else ("bm25_search", "visit")
    search = SearchBm25(name=name_search, snippet=snippet).bind(
...
def test_tools_tuple_query_biased_is_bm25q_search_and_visit_q():
    box, _ = _toolbox((), snippet=TermWindow())
    assert box.tools == ("bm25q_search", "visit_q")


# --- 3. snippet=OpeningLine(): byte-parity with the plain default rendering ----------------------
