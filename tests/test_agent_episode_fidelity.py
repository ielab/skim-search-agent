...
def test_engine_built_only_for_its_arm():
    ...
    code = AgentRetriever(policy_factory=lambda: None, toolset=("search", "fetch"),
                          domain="code").index(units)
    base = AgentRetriever(policy_factory=lambda: None, toolset=("bm25_search", "visit"),
                          domain="general").index(units)
    assert code._bql is not None and code._bm25 is None
    assert base._bm25 is not None and base._bql is None
