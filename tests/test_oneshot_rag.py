"""scripts/oneshot_rag.py: the no-agent-loop one-shot RAG baseline (retrieve top-k, stuff
into one prompt, one model call, parse <answer>). Tested here via its prompt-builder,
answer-parser, retrieval dispatch, token-accounting, and `run_instance`/`main` resume logic,
with a fake generate() (no API call) and, where a real tokenizer helps, an offline HF cache
(no network).
"""
