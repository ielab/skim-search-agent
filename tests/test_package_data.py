...
resolves its prompt.md next to its own task.py, a tool its manual next to its tool.py. A wheel
without them cannot render a prompt."""
import re
import subprocess
import sys
import zipfile
...
def test_built_wheel_contains_the_prompt_files(tmp_path):
    ...
    assert any(re.match(r"agent_search/tasks/[^/]+/prompt\.md$", n) for n in names)
    assert any(re.match(r"agent_search/tools/[^/]+/.*\.md$", n) for n in names)
